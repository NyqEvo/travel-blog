...
function CalendarPicker(element, options) {
  ...
  this.listOfAllDaysAsText = [...Array(7).keys()].map(
    this._toTranslatedWeekday(
      beginningOfYearOnMonday,
      locale,
      options.showShortWeekdays
    )
  );
  this.listOfAllMonthsAsText = [...Array(12).keys()].map(
    this._toTranslatedMonth(beginningOfYearOnMonday, locale)
  );
...
CalendarPicker.prototype._toTranslatedWeekday = function (
  beginningOfYearOnMonday,
  locale,
  showShortWeekdays
) {
  ...
  return function (dayOfWeekIndex) {
    return new Intl.DateTimeFormat(locale, { weekday: weekdayFormat }).format(
      new Date(
        beginningOfYearOnMonday.getFullYear(),
...
CalendarPicker.prototype._toTranslatedMonth = function (
  beginningOfYearOnMonday,
  locale
) {
...
CalendarPicker.prototype._insertCalendarIntoWrapper = function () {
  ...
  var handleSelectedElement = (event) => {
    if (
      ...
      Array.from(this.calendarGrid.querySelectorAll(".selected")).forEach(
...
CalendarPicker.prototype._toggleNavigationButtons = function () {
  this.previousMonthArrow.toggleAttribute(
    "disabled",
    !!this.min && this._beginningOfMonth(this.date) <= this.min
  );
  this.nextMonthArrow.toggleAttribute(
    "disabled",
    !!this.max && this._endOfMonth(this.date) >= this.max
  );
};
